...
if "selected_history_idx" not in st.session_state:
    st.session_state.selected_history_idx = 0  # 제출용 선택 인덱스
...
with col_vis:
    ...
    if st.session_state.step_idx == max_step:
        ...
        if st.button("📊 현재 정렬 결과를 대시보드에 기록하기"):
            ...
            # 새 데이터 추가 시 자동으로 해당 항목을 선택 상태로 설정
            st.session_state.selected_history_idx = len(st.session_state.history) - 1
...
# ---------------------------------------------------------
# 5. 성능 비교 대시보드 및 제출용 데이터 선택 (개선)
# ---------------------------------------------------------
st.header("📈 알고리즘 성능 비교 대시보드")
st.caption("실험 결과를 한눈에 비교하고, 아래 답안과 함께 구글 시트로 제출할 데이터를 선택하세요.")

if st.session_state.history:
    df_history = pd.DataFrame(st.session_state.history)
    st.dataframe(df_history, use_container_width=True)
    
    # 🎯 제출할 대시보드 데이터 선택 라디오 버튼
    options = [
        f"[{i+1}번 기록] {rec['알고리즘']} | {rec['데이터 조건']} (N={rec['데이터 크기(N)']}) | 비교: {rec['총 비교 횟수']}회, 교환: {rec['총 교환 횟수']}회"
        for i, rec in enumerate(st.session_state.history)
    ]
    
    selected_option = st.radio(
        "📌 구글 시트에 답안과 함께 제출할 실험 기록을 선택하세요:",
        options,
        index=min(st.session_state.selected_history_idx, len(options)-1)
    )
    
    # 선택된 라디오 버튼의 인덱스 저장
    st.session_state.selected_history_idx = options.index(selected_option)
    
    if st.button("🗑️ 대시보드 기록 전체 초기화"):
        st.session_state.history = []
        st.session_state.selected_history_idx = 0
        st.rerun()
else:
    st.info("아직 대시보드에 기록된 실험 결과가 없습니다. 정렬을 끝까지 감상한 후 [대시보드에 기록하기] 버튼을 누르면 여기에 저장됩니다!")

st.divider()

# ---------------------------------------------------------
# 6. 개념 기반 탐구 답안 제출 (선택된 대시보드 기록과 함께 제출)
# ---------------------------------------------------------
...
with st.form(key="student_answer_form"):
    ...
    # 선택된 데이터 미리보기 안내
    if st.session_state.history:
        curr_selected = st.session_state.history[st.session_state.selected_history_idx]
        st.info(f"📌 **제출 예정 실험 데이터:** {curr_selected['알고리즘']} / {curr_selected['데이터 조건']} (비교: {curr_selected['총 비교 횟수']}회, 교환: {curr_selected['총 교환 횟수']}회)")
    else:
        st.warning("⚠️ 선택된 실험 데이터가 없습니다. (대시보드 미기록 상태로 제출됩니다.)")
    ...
    submitted = st.form_submit_button("🚀 선택한 결과 및 답안 구글 시트로 제출하기")
    ...
    if submitted:
        if not student_info:
            ...
            # 선택된 대시보드 행 데이터 추출
            if st.session_state.history:
                target_record = st.session_state.history[st.session_state.selected_history_idx]
            else:
                target_record = {}
            
            payload = {
                "student_info": student_info,
                "algorithm": target_record.get("알고리즘", "기록 없음"),
                "data_state": target_record.get("데이터 조건", "-"),
                "data_size": target_record.get("데이터 크기(N)", "-"),
                "comp_count": target_record.get("총 비교 횟수", 0),
                "swap_count": target_record.get("총 교환 횟수", 0),
                "calc_time": target_record.get("순수 연산 시간", "-"),
                "q1": q1,
                "q2": q2,
                "q3": q3
            }
            ...
            try:
                ...
                if response.status_code == 200:
                    st.success(f"🎉 {student_info} 학생의 답안과 선택한 실험 결과가 구글 시트로 성공적으로 제출되었습니다!")
                    st.balloons()
                else:
